...
import { DOCTOR, SERVICES, SCHEDULE, INSURANCE, PAYMENT } from '../constants.js'
      ...
      <nav className={`${styles.nav} ${scrolled ? styles.navScrolled : ''}`}>
        <div className={styles.navInner}>
          ...
          <ul className={`${styles.navLinks} ${menuOpen ? styles.navOpen : ''}`}>
            {[['sobre-mi','Sobre mí'],['servicios','Servicios'],['horarios','Horarios'],['ubicacion','Ubicación']].map(([id,label]) => (
              <li key={id}>
                <button className={styles.navLink} onClick={() => scrollTo(id)}>{label}</button>
              </li>
            ))}
          </ul>
          <button className={styles.navCta} onClick={() => navigate('/agendar')}>
            📅 Agendar Cita
          </button>
      ...
      <section className={styles.hero}>
        ...
        <div className={styles.heroInner}>
          <div className={styles.heroContent}>
            ...
            <h1 className={styles.heroTitle}>
              Cuidamos la<br />
              <span className={styles.heroAccent}>salud respiratoria</span><br />
              de tus hijos
            </h1>
            <p className={styles.heroSubtitle}>
              Especialista en neumología pediátrica con más de una década de experiencia.
              Diagnóstico preciso, trato humano, resultados reales.
            </p>
            <div className={styles.heroActions}>
              <button className={styles.btnHero} onClick={() => navigate('/agendar')}>
                📅 Agendar una cita
              </button>
              <button className={styles.btnHeroGhost} onClick={() => scrollTo('servicios')}>
                Ver servicios →
              </button>
            </div>
          ...
          <div className={styles.heroVisual}>
            <div className={styles.avatarWrap}>
              ...
              <div className={styles.avatarCircle}>
                <img src="/doc.png" alt="Dr. Carlos Mendoza" className={styles.avatarImg} />
              </div>
        ...
        <div className={styles.heroWave}>
          <svg viewBox="0 0 1440 80" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none">
            <path d="M0 80L60 68C120 56 240 32 360 26C480 20 600 32 720 38C840 44 960 44 1080 36C1200 28 1320 8 1380 0L1440 0V80H0Z" fill="white"/>
          </svg>
        </div>
      </section>
      ...
      <section id="servicios" className={styles.services}>
        <div className={styles.sectionInner}>
          ...
          <p className={styles.sectionSub}>
            Diagnóstico y tratamiento integral de enfermedades respiratorias en niños y adolescentes.
          </p>
      ...
      <section id="horarios" className={styles.schedule}>
        <div className={styles.sectionInner}>
          ...
          <div className={styles.scheduleNote}>
            📋 Atención por <strong>orden de llegada</strong> según fecha agendada.
            Le recomendamos llegar unos minutos antes de la hora seleccionada.
          </div>
        </div>
      </section>
      ...
      <section className={styles.cta}>
        <div className={styles.ctaInner}>
          ...
          <p className={styles.ctaDesc}>
            Sin filas. Sin esperas. Selecciona tu fecha y hora disponible en segundos.
          </p>
          <button className={styles.ctaBtn} onClick={() => navigate('/agendar')}>
            📅 Agendar ahora
          </button>
        </div>
      </section>
...
function SectionLabel({ children }) {
  const [ref, visible] = useReveal()
  return (
    <div ref={ref} className={`${styles.sectionLabel} ${visible ? styles.visible : ''}`}>
      <span className={styles.labelLine} />{children}
    </div>
  )
}
...
function StatCard({ num, suffix, label, index }) {
  const [ref, visible] = useReveal()
  return (
    <div ref={ref} className={`${styles.statCard} ${visible ? styles.visible : ''}`}
      style={{ transitionDelay: `${index * 100}ms` }}>
      <div className={styles.statNum}>
        {visible ? <AnimatedCounter target={num} suffix={suffix} /> : `0${suffix}`}
      </div>
...
function AboutText() {
  const [ref, visible] = useReveal()
  return (
    <div ref={ref} className={`${styles.aboutText} ${visible ? styles.visible : ''}`}>
      <p>{DOCTOR.about}</p>
      <p style={{ marginTop: '1rem' }}>
        Mi enfoque combina la evidencia científica más actualizada con un trato cercano y empático,
        porque entiendo que cuando un niño está enfermo, toda la familia necesita apoyo.
      </p>
      <div className={styles.aboutTags}>
        {['Neumología pediátrica','Alergias respiratorias','Trastornos del sueño','Asma infantil'].map(t => (
...
function AboutFeatures() {
  return (
    <div className={styles.aboutFeatures}>
      {[
        { icon: '🏥', title: 'Consultorio equipado', desc: 'Instalaciones modernas para diagnóstico preciso y confiable.' },
        { icon: '💊', title: 'Tratamiento personalizado', desc: 'Plan de tratamiento adaptado a cada paciente y su familia.' },
        { icon: '📋', title: 'Seguimiento continuo', desc: 'Acompañamos a la familia durante todo el proceso de recuperación.' },
      ].map((f, i) => {
        const [ref, visible] = useReveal()
        return (
          <div key={i} ref={ref}
...
function ServiceCard({ service, delay }) {
  const [ref, visible] = useReveal()
  return (
    <div ref={ref} className={`${styles.serviceCard} ${visible ? styles.visible : ''}`}
...
function InsuranceCard({ icon, title, desc, highlight, pills }) {
  const [ref, visible] = useReveal()
  return (
    <div ref={ref} className={`${styles.insuranceCard} ${visible ? styles.visible : ''}`}>
      ...
      {highlight && <div className={styles.insuranceHighlight}>✅ {highlight}</div>}
      {pills && (
        <div className={styles.insurancePills}>
          {pills.map((p, i) => <span key={i} className={styles.insurancePill}>{p}</span>)}
        </div>
      )}
    </div>
  )
      ...
      {DAY_NAMES.map((day, i) => {
        ...
        return (
          <div key={i} ref={ref}
            ...
            <span className={`${styles.scheduleDot} ${slot ? styles.dotGreen : styles.dotGray}`} />
            <span className={styles.scheduleDay}>{day}</span>
            <span className={styles.scheduleTime}>
              {slot ? `${minutesToTime(slot.start)} – ${minutesToTime(slot.end)}` : 'No disponible'}
            </span>
          </div>
        )
      })}
...
function LocationInfo() {
  const [ref, visible] = useReveal()
  return (
    <div ref={ref} className={`${styles.locationInfo} ${visible ? styles.visible : ''}`}>
      {[
        { icon: '📍', label: 'Dirección', value: `${DOCTOR.address.street}, ${DOCTOR.address.area}` },
        { icon: '🏥', label: 'Consultorio', value: DOCTOR.address.clinic },
        { icon: '📞', label: 'Teléfono', value: DOCTOR.phone },
        { icon: '🚗', label: 'Estacionamiento', value: 'Disponible en el edificio para pacientes' },
      ].map((d, i) => (
...
function MapBlock() {
  ...
  return (
    <div ref={ref} className={`${styles.mapWrap} ${visible ? styles.visible : ''}`} onClick={handleClick}>
      <div className={styles.mapPlaceholder}>
        ...
        <div className={styles.mapText}>
          <strong>{DOCTOR.address.street}</strong>
          <span>{DOCTOR.address.area}</span>
          <span className={styles.mapCta}>Abrir en Google Maps →</span>
        </div>
      </div>
    </div>
  )
...
function CookieBanner() {
  const [visible, setVisible] = useState(() => !localStorage.getItem('cookies_ok'))
  if (!visible) return null
  return (
    <div className={styles.cookie}>
      <p className={styles.cookieText}>
        🍪 Usamos cookies para mejorar tu experiencia.{' '}
        <a href="#" className={styles.cookieLink}>Política de privacidad</a>
      </p>
      <div className={styles.cookieBtns}>
        <button className={styles.cookieAccept} onClick={() => { localStorage.setItem('cookies_ok','1'); setVisible(false) }}>Aceptar</button>
        <button className={styles.cookieReject} onClick={() => setVisible(false)}>Rechazar</button>
      </div>
    </div>
  )
}
